...
import type { PageFrontmatter, ThemeDefinition, ThemeName } from "../content/types";
...
export function usePagePresentation(frontmatter: PageFrontmatter) {
  useEffect(() => {
    let cancelled = false;
    let mediaQuery: MediaQueryList | null = null;

    document.documentElement.style.setProperty("--page-font-size", frontmatter.fontsize);
    applyFont(frontmatter.font);

    const updateTheme = () => {
      const resolvedTheme = resolveThemeName(frontmatter.theme);
